(function () {
  ...
  const cardClassPattern =
    /(rounded-2xl|rounded-3xl|rounded-xl|glass-card|lifestyle-card|pipeline-step|phase-tab|prostate-card)/;
  const ctaClassPattern = /(bg-red-600|bg-emerald-700|bg-emerald-800|bg-white\/10|bg-white\/20|prostate-btn)/;
  ...
  function assignAosAttributes() {
    ...
    sections.forEach((section, index) => {
      if (!section.dataset.aos) {
        section.dataset.aos = index === 0 ? "fade-in" : "fade-up";
        section.dataset.aosDuration = "800";
        section.dataset.aosDelay = String(Math.min(index * 40, 280));
    ...
    cards.forEach((card, index) => {
      ...
      if (!card.dataset.aos) {
        ...
        card.dataset.aosDuration = "650";
        card.dataset.aosDelay = String(Math.min((index % 6) * 50, 300));
      }
    });
    ...
    ctas.forEach((cta) => {
      ...
      if (!cta.dataset.aos) {
        ...
        cta.dataset.aosDuration = "550";
        cta.dataset.aosDelay = "120";
      }
    });
  }
  ...
  function bindCardMicroInteractions() {
    ...
    cards.forEach((card) => {
      ...
      card.addEventListener("pointermove", (event) => {
        ...
        const rx = ((x - cx) / Math.max(cx, 1)) * 2.5;
        const ry = ((cy - y) / Math.max(cy, 1)) * 2.2;
  ...
  function bindSmoothSectionTransitions() {
    if (reduceMotion) return;
